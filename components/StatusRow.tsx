...
const StatusRow: React.FC<StatusRowProps> = ({ product, id, isFeaturesOpen, onToggleFeatures }) => {
    ...
    return (
        <div
            id={id}
            className="group relative overflow-hidden bg-zinc-900/40 border border-zinc-800 transition-all duration-500 rounded-lg mb-4 hover:border-brand-500/50 hover:shadow-[0_0_30px_rgba(99, 102, 241, 0.1)]"
        >
                        ...
                        <div className="grid grid-cols-2 sm:flex sm:flex-wrap xl:grid xl:grid-cols-2 gap-2">
                            {product.features && product.features.length > 0 && (
                                <button
                                    ...
                                    className={`flex items-center justify-center gap-2 px-4 py-2 rounded-md border text-xs font-bold transition-all uppercase font-mono ${isFeaturesOpen
                            ...
                            {product.youtubeVideoId && (
                                <button
                                    onClick={toggleGuide}
                                    className={`flex items-center justify-center gap-2 px-4 py-2 rounded-md border text-xs font-bold transition-all uppercase font-mono ${showGuide
                                        ? 'bg-zinc-800 text-white border-zinc-700'
                                        : 'bg-zinc-900/50 text-zinc-400 border-zinc-800 hover:text-white hover:border-zinc-700'
                                        }`}
                                >
            ...
            <AuthModal
                ...
                panelName={product.name}
                authorizedUsers={product.authorizedUsers}
            />
